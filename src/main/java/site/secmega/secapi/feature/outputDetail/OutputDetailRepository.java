...
public interface OutputDetailRepository extends JpaRepository<OutputDetail, Long>, JpaSpecificationExecutor<OutputDetail> {
    ...
    @Query("SELECT COALESCE(SUM(o.goodQty), 0) from OutputDetail o where o.deletedAt is null and o.workOrder.isActive = true")
    long sumGoodOutputQtyActive();
    ...
    @Query("""
            select COALESCE(SUM(o.goodQty), 0) from OutputDetail o
            where o.fromLine.department.processNo = ?1 and o.workOrder.isActive = true""")
    Integer countByFromLine_Department_ProcessNoAndWorkOrder_IsActiveTrue(Integer processNo);
